...
describe("parse()", () => {
  ...
  it.only("Should be able to parse enclosures.", () => {
    ...
    expect(output("1 + 2(2 + 3)")).to.be.equal(null);
    expect(output("1(2 + 3)")).to.be.equal(null);
    expect(output(":test()")).to.be.equal(null);
    expect(output(":test(1)")).to.be.equal(null);
  });

  it.only("Should be able to parse enclosures and multiple arguments.", () => {
    reset();
    expect(output("1[] + 2")).to.be.equal(
      "int|1:0,list|[:1,int|2:0,add|+:2"
    );
    expect(output("(1 + 3) * 6")).to.be.equal(
      "int|1:0,int|3:0,add|+:2,int|6:0,mul|*:2"
    );
    ...
    expect(output("1 + 2[]")).to.be.equal(
      "int|1:0,int|2:0,list|[:1,add|+:2"
    );
    expect(output("1[2] + 3")).to.be.equal(
      "int|1:0,int|2:0,list|[:2,int|3:0,add|+:2"
    );
    expect(output("1[2, 3] + 3")).to.be.equal(
      "int|1:0,int|2:0,int|3:0,list|[:3,int|3:0,add|+:2"
    );
    expect(output("3 + 1[2, 3]")).to.be.equal(
      "int|3:0,int|1:0,int|2:0,int|3:0,list|[:3,add|+:2"
    );
  });
});
