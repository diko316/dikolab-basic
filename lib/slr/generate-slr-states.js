...
function getAndMergeManifest() {
  ...
  mainLoop: for (; length--; c++) {
    ...
    case "--config":
      if (!length) {
        reportError(2, "--config switch requires Config file parameter.");
      }
      length--;
      param = args[++c];
      config = getJSON(param);
      if (!config) {
...
function generate() {
  ...
  if (!createJSONFile(target, content, options.optimized)) {
    reportError(5);
  }

  console.error("Output size: ", (getFileSize(target) * 0.001).toFixed(2), " Kilobytes.");
